            ...
            <div class="col-12 mb-5">
                <div class="row m-5 pb-0 mb-0 justify-content-center align-items-center userWorkout flex-wrap">
                    ...
                    <div id="mainToggleDiv" class="collapse row m-5 p-3 justify-content-center align-items-center userWorkoutDays flex-wrap">
                        @if($days->isEmpty())
                            <h1 class="text-white m-3 text-uppercase text-center">{{ __('you don\'t have workouts yet') }}</h1>
                        @else
                        @foreach($days as $day)
                        <div class="workoutDays  p-3" id="dayDiv{{ $day->workout_day_id }}">
                            <div class="row d-flex justify-content-center align-items-center">
                                <div class="col-md-3 p-0 m-0 d-flex justify-content-center align-items-center">
                                    <img src="{{ asset($day->workoutDayImg) }}" alt="{{ __('Day Image') }}" class="personalImg">
                                </div>
                            ...
                            <div id="toggleDiv{{ $day->workout_day_id }}" class="collapse row justify-content-center align-items-center flex-wrap userWorkoutExercises">
                                @if(empty($day->exercises))
                                    <h1 class="text-white m-3 text-uppercase text-center">{{ __('you don\'t have Exercises on this day') }}</h1>
                                @endif
                                @foreach($day->exercises as $exercise)
                                <div class="row p-3 d-flex justify-content-center align-items-center workoutDayExercises">
                                    <div class="col-md-3 p-0 m-0 d-flex justify-content-center align-items-center">
                                        <img src="{{ asset($exercise->exerciseImg) }}" alt="{{ __('Exercise Image') }}" class="personalImg ">
                                    </div>
                                    <div class="col-md-5 p-0 h-75">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <p class="text-white"><strong>{{ __('Exercise Name:') }}</strong> {{ $exercise->exercise_name }}</p>
                                                <p class="text-white"><strong>{{ __('Exercise Equipment type:') }}</strong> {{ __($exercise->exercise_type) }}</p>
                                                <p class="text-white "><strong>{{ __('Exercise Difficulty:') }}</strong> {{ __($exercise->exercise_strength_level) }}</p>
                                            </div>
                                            <div class="col-md-6">
                                    ...
                                    <div class="col-md-2 p-3 d-flex justify-content-center align-items-center">
                                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#updateExerciseModal{{ $exercise->exercise_workout_id}}">{{ __('edit') }}</button>
                                    </div>
                                </div>
